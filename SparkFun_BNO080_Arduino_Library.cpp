...
bool BNO080::begin(uint8_t deviceAddress, DWire &wirePort)
{
...
void BNO080::enableDebugging(DSerial &debugPort)
{
...
void BNO080::parseInputReport(void)
{
  ...
  dataLength &= ~(1 << 15); //Add the MSbit. This bit indicates if this package is a continuation of the last.
  ...
  else if (shtpData[5] == SHTP_REPORT_COMMAND_RESPONSE)
  {
	  ...
	  if(command == COMMAND_ME_CALIBRATE)
	  {
		_debugPort->println("ME Cal report found!");
...
bool BNO080::readFRSdata(uint16_t recordID, uint8_t startLocation, uint8_t wordsToRead)
{
  ...
  while (1)
  {
    ...
    {
      if (_printDebug == true) _debugPort->println("metaData array over run. Returning.");
...
bool BNO080::waitForI2C()
{
  ...
  if (_printDebug == true) _debugPort->println("I2C timeout");
  return (false);
}
...
bool BNO080::receivePacket(void)
{
    ...
    _i2cPort->requestFrom((uint8_t)_deviceAddress, (uint16_t)4); //Ask for four bytes to find out how much data we need to read
...
bool BNO080::getData(uint16_t bytesRemaining)
{
  ...
  while (bytesRemaining > 0)
  {
    ...
    _i2cPort->requestFrom((uint8_t)_deviceAddress, (uint16_t)(numberOfBytesToRead + 4));
...
void BNO080::printPacket(void)
{
  if (_printDebug == true)
  {
    ...
    _debugPort->print("Header:");
    for (uint8_t x = 0 ; x < 4 ; x++)
    {
      _debugPort->print(" ");
      if (shtpHeader[x] < 0x10) _debugPort->print("0");
    ...
    _debugPort->print(" Body:");
    for (uint8_t x = 0 ; x < printLength ; x++)
    {
      _debugPort->print(" ");
      if (shtpData[x] < 0x10) _debugPort->print("0");
      _debugPort->print(shtpData[x], HEX);
    }

    if (packetLength & 1 << 15)
    {
      _debugPort->println(" [Continued packet] ");
      packetLength &= ~(1 << 15);
    }
    ...
    if (shtpHeader[2] == 0) _debugPort->print("Command");
    else if (shtpHeader[2] == 1) _debugPort->print("Executable");
    else if (shtpHeader[2] == 2) _debugPort->print("Control");
    else if (shtpHeader[2] == 3) _debugPort->print("Sensor-report");
    else if (shtpHeader[2] == 4) _debugPort->print("Wake-report");
    else if (shtpHeader[2] == 5) _debugPort->print("Gyro-vector");
...
/* @author: Mohammed
 * @params: uint8_t seconds: duration (in seconds) where program is halted
 *
 * using assembly instruction _asm(" nop") to do nothing for exactly one cycle
 */

void BNO080::delay(uint8_t seconds) {

    uint32_t numCycles = (CS_getMCLK() * seconds)/1000; // number of cycles per second
