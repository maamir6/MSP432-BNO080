...
class BNO080 {
  public:
    ...
    bool begin(uint8_t deviceAddress, DWire &wirePort); //By default use the default I2C addres, and use Wire port

    void enableDebugging(DSerial &debugPort); //Turn on debug printing. If user doesn't specify then Serial will be used.
  ...
  private:
    ...
    //Variables
    DWire *_i2cPort; //The generic connection to user's chosen I2C hardware
    uint8_t _deviceAddress; //Keeps track of I2C address. setI2CAddress changes this.

    DSerial *_debugPort; //The stream to send debug messages to if enabled. Usually Serial.
    ...
    // @mohammed: created these functions for compatibility with msp
